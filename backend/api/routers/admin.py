...
async def approve_or_reject_request(
    ...
    req_body: dict = json.loads(req.body) if req.body else {}

    # On approval of a leave type — deduct days from the balance
    if body.status == "approved" and req.type in ("vacation", "sick", "pto"):
        days = float(req_body.get("days", 0))
        start_date_str: str = req_body.get("start_date", "")
        try:
            year = datetime.fromisoformat(start_date_str).year if start_date_str else datetime.now(UTC).year
        except ValueError:
        ...
        if days > 0:
            ...
            if balance is not None:
                balance.used_days = cast(float, balance.used_days) + days
    ...
    return ApproveRejectResponse(
        ...
        updated_balances=[
            BalanceSnapshot(
                leave_type=cast(str, b.leave_type),
