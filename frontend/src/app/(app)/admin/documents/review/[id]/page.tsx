...
import { ArrowLeft, CheckCircle, FileText, Loader2, RefreshCw } from "lucide-react";
...
export default function DocumentReviewPage() {
  ...
  async function handleApprove(e: React.FormEvent) {
    ...
    try {
      const result = await api.approveExtraction(documentId, {
        notes: form.notes,
        pto_days:
          form.pto_days !== "" ? Number(form.pto_days) : null,
        sick_days:
          form.sick_days !== "" ? Number(form.sick_days) : null,
        vacation_days:
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl space-y-8 p-6">
      ...
      {/* Review form */}
      {canApprove && (
        <form onSubmit={(e) => void handleApprove(e)} className="space-y-6">
          <div>
          ...
          {missingFields.length > 0 && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-900/20">
              ...
              <ul className="mt-1 list-inside list-disc space-y-0.5">
                {missingFields.map((f) => (
                  <li key={f} className="text-[12px] text-amber-600 dark:text-amber-400">
                    {f}
                  </li>
                ))}
              </ul>
          ...
          {success && (
            <div className="flex items-start gap-2 rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-900 dark:bg-green-900/20">
              ...
              <p className="text-[12px] text-green-700 dark:text-green-300">
                Policy applied to{" "}
                <strong>{success.employees_updated} employee(s)</strong> in the{" "}
                <strong>{success.department}</strong> department for{" "}
